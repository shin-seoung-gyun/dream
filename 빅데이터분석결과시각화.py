...
data = pd.read_csv('./지리정보시각화/lotto.csv',encoding='utf-8',engine='python')
print(data.head())
...
addr['시도'].replace("광주","광주광역시",inplace=True)
...
# idx = addr[addr['시도']=='동행복권(dhlottery.co.kr)']/
# addr.drop(idx,inplace=True)

print(addr['시도'].unique())
print(addr['군구'].unique())
addr['군구'].replace("나성북로","세종특별자치시",inplace=True)
addr['군구'].replace("새롬중앙1로","세종특별자치시",inplace=True)
addr['군구'].replace("조치원로","연기군",inplace=True)
addr['군구'].replace("행복10길","연기군",inplace=True)
print(addr['군구'].unique())
...
draw_blockMap(data_all, 'mc_ratio','행정구역별 인구수 대비 lotto 비율', 'Reds')
