...
#리스트 컴피리헨션 이 문법이 없을땐 map을 사용했었음 익숙해지면 많이 편해질듯..
...
newList = list(map(lambda x,y:x+y, numList1,numList2))##map은 입력값을 바꿔주는 함수(람다식과 같이 사용할 경우)
print(newList)
...
#->['사과-10','딸기-8','배-20']
##map을 이용한 방법 맵에 람다식을 사용하여 값을 바꿔주고 새로운 리스트에 추가
