...
#해당 클러스터의 샘플로 지정한다.
#(3) 클러스터에 속한 샘플의 평균값으로 클러스터 중심을
#변경한다.
#(4) 클러스터 중심에 변화가 없을때까지 (2)번으로 돌아가
#반복한다.
...
#라벨별 시각화
label0 = fruits[km.labels_==0][4].reshape(100,100)
plt.imshow(label0, cmap='gray_r')
plt.show() #label0은 파인애플

label1 = fruits[km.labels_==1][40].reshape(100,100)
plt.imshow(label1, cmap='gray_r')
plt.show() #label1은 바나나

label2 = fruits[km.labels_==2][59].reshape(100,100)
...
draw_fruits(fruits[km.labels_==0]) #파인애플 주로
draw_fruits(fruits[km.labels_==1]) #모두 바바나
draw_fruits(fruits[km.labels_==2]) #모두 사과

#클러스터 중심 (라벨에 따른 100*100 픽셀별 평균)
km.cluster_centers_.shape #(3, 10000)
draw_fruits(km.cluster_centers_.reshape(-1,100,100), 
    fig_size_ratio=3)

#훈련 데이터 샘플에서 클러스터별 중심까지 거리값 거리
...
#최적의 클러스터 평균값 찾기 위한 알고리즘이 반복한 횟수
km.n_iter_ #4

#-----------------------------------------------------
#실전에서는 클러스터 개수를 알수가 없다.
#최적의 K값을 찾기
#(1) 엘보우 방법
#(2) 실루엣 방법
...
#클러스터에 속한 샘플이 얼마나 중심에 가깝게 모여있는지 생각
#할 수 있다.

#클러스터 늘어나면(중심이 많아지면) 이너셔가 줄어듬
#엘보우 방법은 클러스터 개수를 늘려가면서 이너셔의 변화를 관찰

#감소하는 속도가 꺽이는 지점이 있다.
#이 이후 부터는 클러스터 개수를 늘려도 잘 밀집된 정도가 크게
#개선되지 않음
...
#를 나타내는 응집력 a(i)와 데이터가 다른 클러스터로부터
#얼마나 떨어져있는가를 나타내는 클러스터 분리도 b(i)를
#이용한 실루엣 계수 s(i)를 계산한다.
#실루엣 계수는 -1~1사이의 값을 가지며 1에 가까울 수록 좋은
#군집화
