...
import glob
from keras.backend import dropout, placeholder #여러파일을 한꺼번에 처리할수있도록 하는 모듈
...
#1)데이터 수집
path = './data/poke'
targets = ['Arcanine', 'Bellsprout', 'Lapras', 'Shellder', 'Venusaur']
n_class = len(targets)
...
np.save(path+'/pokeObj.npy', xyData)
...
base_model = DenseNet201(include_top=False, input_shape=X_train.shape[1:], weights='imagenet', pooling='avg')
base_model.trainable = False
model.add(base_model)

# model.add(Conv2D(32,kernel_size=3, activation='relu',padding='same'))
# #필터(커널)크기 = 3,3,3
# #피처맵의 크기 100,100,3,32
# model.add(MaxPool2D(2))
# #풀링층의 크기 : 2,2,3
# #풀링층을 거치고 난 후 크기 : 50,50,3,32
...
#모델 훈련 하기
#베스트 훈련모델을 저장
checkpoint_cb = keras.callbacks.ModelCheckpoint('./best-cnn-poke.h5', save_best_only=True)
...
#----------------------------------------------------------------------------------------------------------------
#새로운 데이터 예측
img = Image.open(path + './arcanine.jpg')
img = img.convert("RGB")
img = img.resize((image_w,image_h))
data = np.asarray(img)/255
data = data.reshape(1,image_w,image_h,3)
data.shape
np.argmax(model.predict([data]))#4
model.predict([data])
#array([[9.9999464e-01, 1.6360400e-09, 5.3972235e-06, 5.2509690e-09,
        # 1.3371708e-08]], dtype=float32)
model.evaluate(X_test,Y_test)# [0.05381030589342117, 0.9820359349250793]
