...
# ^ : 문자열의 시작을 의미
# $ : 문자열의 끝을 의미
# ? : 앞문자가 있어도 되고 없어도 됨, 최소검색할때 반복자 다음에 붙여서 사용하기도 함  ex) appl?e=>apple , appe(o)
# (?!) : 대소문자를 구문하지 않는다.
...
# ex) 김현수(o), 배철수(x)
text = '김현수, 배철수'
m = re.search(r'(김[가-힣]+)',text)
print(m.group(1))
##(2) 휴대전화 끝자리 4번호 ****로 바꾸기
text2 = '010-1234-1234'
subStr = re.sub(r'(\d{3}-\d{4})-(\d{4})','\g<1>-****',text2)
print(m)
##(3) 괄호가 없는 사칙연산 수식이 있을때 정규표현식을 활용하여,숫자만 리스트로 사칙연산만 리스트로 각각 가져오시오
text3='12*4+5-60*3'
m = re.findall(r'\d+',text3)
print(m)
k = re.findall(r'[\*\+\-\/]+',text3)
print(k)

##(4) 패턴 일치 검사 날짜가 yyyy-mm-dd로 주어졌을때 해장하는 패턴에 맞는지 검사하시오
##ex)2020-03-04(0),2020-3-4(x)
##2021-04-32(x),2021-13-18(x)
text = '2020-03-44'
m = re.search(r'\d{4}-(?:0[1-9]|1[012])-(?:0[1-9]|[12][0-9]|3[0-1])',text)
if m :
    print(m.group())
else :
    print("일치하지 않는다")
