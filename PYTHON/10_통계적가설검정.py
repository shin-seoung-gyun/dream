...
#5.카이제곱검정 - 독립성 검정
#귀무가설 H0 :두 변수 X와 Y에 관해서 X와 Y가 독립이다.
#대립가설 H1 : 두 변수 X와 Y에 관해서 X와 Y가 독립이 아니다.
#독립성검정은 카이제곱검정이라고 부름

#ex) 광고의 종류와 상품구입 유무가 독립인지 여부
#H0 : 광고 A를 보든 B를 보든 구입 비율에 변화가 없다.
#H1 : 광고에 따라 구입 비율에 유의한 차이가 있다.

ad_df = pd.read_csv('./data/ch11_ad.csv')
ad_df.info()
ad_df.head()
n=len(ad_df)
#교차집계표(분할표) - 도수분포표의 2변수 버전
...
#상품구입 비율은 A광고쪽이 크다. 유의한 차이가 있는지 검정

#상품구입 여부 합계
n_not, n_yes = ad_cross.sum(axis=0)#행방향 합계
n_not, n_yes #광고와 상관없이 구입비율은 10%

#광고를 본사람의 합계값
n_adA, n_adB = ad_cross.sum(axis=1)#열방향 합계
n_adA, n_adB 

#광고와 구입이 독립이라면 
#A광고를 본 사람의 구입수 400명중 10% => 40명
#B광고를 본 사람의 구입수 600명중 10% => 60명
#기대도수 : 광고와 구입이 독립인 변수일때 기대되는 도수
#관측도수 : 실제로 관측된 데이터

#기대도수 계산
n_aDoEf= n_adA*n_yes/(n_yes+n_not)
n_bDoEf= n_adB*n_yes/(n_yes+n_not)
n_aNDoEf= n_adA*n_not/(n_yes+n_not)
n_bNDoEf= n_adB*n_not/(n_yes+n_not)
ad_ef = pd.DataFrame({'하지 않았다' : [n_aNDoEf,n_bNDoEf],'했다':[n_aDoEf,n_bDoEf]}, index=['A','B'])
ad_ef

##카이제곱검정 실시
#기대도수와 관측도수의 차이를 측정함으로써 검정 수행
#검정통계량 구하기
y=(ad_cross-ad_ef)**2/ad_ef
y = y.sum(axis=1)#열방향 합계
y = y.sum(axis=0)#행방향 합계
y
#Y는 자유도 1인 카이제곱분포를 근사적으로 따른다고 알려져 있다.
rv = stats.chi2(1)

#p값 구하기
1-rv.cdf(y) #0.052807511416113395

#결론 : 귀무가설 채택, 광고 A와 B에 유의한 차이가 인정되지 않는다.

#chi2_contingency 함수로 간단하게 독립성 검사 가능###############################################################################################
...
##모집단의 모분산을 모를때 평균에 관한 검증
#t분포
#=검정통계량 = 표본평균-평균/np.sqrt(s^2/n)) s는 표본평균표준편차
...
#집단1 과 집단 2의 분산이 서로 동일한 것으로 알려져 있는데
#집단 1에서 표본 11개 집단 2에서 8개를 뽑아 분산을 측정했더니 
#각각 15 10 나왔다. 두분안이 서로 다른지 유의 수준 5%로 검정
